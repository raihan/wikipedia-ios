...
class SavedArticlesCollectionViewCell: ArticleCollectionViewCell {
    ...
    override func setup() {
        ...
        registerForTraitChanges([UITraitPreferredContentSizeCategory.self, UITraitDisplayScale.self]) { [weak self] (cell: Self, previousTraitCollection: UITraitCollection) in
            guard let self else { return }
            self.singlePixelDimension = self.traitCollection.displayScale > 0 ? 1.0/self.traitCollection.displayScale : 0.5
            self.configuredTags = []
